...
const signupError = ref<boolean>(false);
  ...
  <div class="w-full h-full flex flex-row justify-center items-center text-center">
    <div class="h-full w-1/2 flex flex-col justify-center gap-6 px-[10%]">
      <div class="flex flex-col gap-4">
        ...
        <p class="text-lg">Welcome! Create an account to access WSA.</p>
      </div>
      ...
      <form class="flex flex-col gap-4" @submit.prevent="handleSignup">
        ...
        <button class="btn bg-[#088395] px-10 text-lg" type="submit">Create Account</button>
        <p v-if="signupError" class="text-red-500 opacity-80">Error: {{ errorMessage }}</p>
      </form>
      <p class="text-lg"
        >New here? <span class="text-[#088395]"><RouterLink to="/login">Sign In</RouterLink></span></p
      >
    </div>
